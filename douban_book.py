    ...
    def export(self, url):
        ...
        if len(book_items) > 0:
            for item in book_items:
                ...
                if ":" in title:
                    title = ":".join(
                        list(map(lambda x: x.strip(), title.split(" : "))))
                meta_data_list = item.find(
                    "div", {"class": "pub"}).text.split(" / ")
                meta_data_list = list(
                    map(lambda x: x.strip(), meta_data_list))  # clean up
                if len(meta_data_list[0]) > 0:
                    ...
                    if publish_date is not None:
                        publishing_company = meta_data_list[meta_data_list.index(
                            publish_date) - 1]
                    else:
